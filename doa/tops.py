...
class TOPS(MUSIC):
    ...
    def _process(self, X):
        """
        Perform TOPS for a given frame in order to estimate steered response spectrum.
        """

        # select reference frequency
        max_bin = np.argmax(np.sum(np.sum(abs(X[:,self.freq,:]),axis=0),axis=1))
        f0 = self.freq[max_bin]
        freq = list(self.freq)
        freq.remove(f0)
        ...
        F = np.zeros((self.num_freq,self.M,self.num_sources), dtype='complex64')
        W = np.zeros((self.num_freq,self.M,self.M-self.num_sources), dtype='complex64')
        for k in range(self.num_freq):
            # subspace decomposition
            F[k,:,:], W[k,:,:], ws, wn = self._subspace_decomposition(C_hat[k,:,:])

        # create transformation matrix
        f = 1.0/self.nfft/self.c*1j*2*np.pi*self.fs*(np.linspace(0,self.nfft/2,self.nfft/2+1)-f0)
        ...
        for n in range(self.num_loc):
            # form D matrix
            D = np.zeros((self.num_sources,(self.M-self.num_sources)*(self.num_freq-1)), dtype='complex64')
            for k in range(self.num_freq-1):
                Uk = np.conjugate(np.dot(np.diag(Phi[k,:,n]), 
                    F[max_bin,:,:])).T
                    # F[max_bin,:,:])).T
                idx = range(k*(self.M-self.num_sources),(k+1)*(self.M-
                    self.num_sources))
                D[:,idx] = np.dot(Uk,W[k,:,:])
