...
class WAVES(MUSIC):
    ...
    def _process(self, X):
        ...
        for k in range(self.num_freq):
            self.P = self._compute_spatial_spectrum(C_hat[k,:,:],self.freq[k])
            self._peaks1D()
        ...
        self.Z = np.empty((self.M,len(self.freq)*self.num_sources), dtype='complex64')
    ...
    def _construct_waves_matrix(self, C_hat, f0, beta):
        for j in range(len(self.freq)):
            ...
            # form WAVES matrix
            idx1 = j*self.num_sources
            idx2 = (j+1)*self.num_sources
